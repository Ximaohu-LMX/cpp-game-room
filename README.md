...
- RankService 以 Redis Sorted Set 为目标结构，当前代码提供内存实现，方便本地演示。
...
- Redis: `ZSET rank:score`，member 为 player_id，score 为玩家积分。

当前源码的 MysqlClient / RedisClient 是可替换的薄封装，默认用内存结构跑通主流程；接真实依赖时只需要替换 storage 层实现。
